...
basis. -/
lemma findim_eq_card_basis {ι : Type w} [fintype ι] {b : ι → V} (h : basis K b) :
...
`findim`. -/
lemma findim_eq_card_basis' [finite_dimensional K V] {ι : Type w} {b : ι → V} (h : basis K b) :
...
lemma equiv_fin {ι : Type*} [finite_dimensional K V] {v : ι → V} (hv : basis K v) :
  ∃ g : fin (findim K V) ≃ ι, basis K (v ∘ g) :=
begin
...
lemma fin_basis [finite_dimensional K V] : ∃ v : fin (findim K V) → V, basis K v :=
...
lemma findim_eq_zero_of_not_exists_basis
  (h : ¬ ∃ s : finset V, basis K (λ x, x : (↑s : set V) → V)) : findim K V = 0 :=
...
lemma basis_of_findim_zero [finite_dimensional K V]
...
lemma basis_of_findim_zero' [finite_dimensional K V]
  (hV : findim K V = 0) : basis K (λ x : fin 0, (0 : V)) :=
basis_of_findim_zero (finset.univ_eq_empty.mp rfl) hV
...
lemma basis_of_span_eq_top_of_card_eq_findim {ι : Type*} [fintype ι] {b : ι → V}
  (span_eq : span K (set.range b) = ⊤) (card_eq : fintype.card ι = findim K V) :
  basis K b :=
⟨linear_independent_of_span_eq_top_of_card_eq_findim span_eq card_eq, span_eq⟩

lemma finset_basis_of_span_eq_top_of_card_eq_findim {s : finset V}
  (span_eq : span K (↑s : set V) = ⊤) (card_eq : s.card = findim K V) :
  basis K (coe : (↑s : set V) → V) :=
basis_of_span_eq_top_of_card_eq_findim
  ((@subtype.range_coe_subtype _ (λ x, x ∈ s)).symm ▸ span_eq)
  (trans (fintype.card_coe _) card_eq)

lemma set_basis_of_span_eq_top_of_card_eq_findim {s : set V} [fintype s]
...
begin
  ...
  obtain ⟨_, b_spans⟩ := set_basis_of_linear_independent_of_card_eq_findim
