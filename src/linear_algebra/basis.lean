...
def finsupp.sum_elim {α β γ : Type*} [has_zero γ]
  (f : α →₀ γ) (g : β →₀ γ) : α ⊕ β →₀ γ :=
finsupp.on_finset
...
@[simp] lemma finsupp.sum_elim_apply {α β γ : Type*} [has_zero γ]
  (f : α →₀ γ) (g : β →₀ γ) (x : α ⊕ β) : finsupp.sum_elim f g x = sum.elim f g x := rfl

lemma finsupp.sum_elim_inl {α β γ : Type*} [has_zero γ]
  (f : α →₀ γ) (g : β →₀ γ) (x : α) : finsupp.sum_elim f g (sum.inl x) = f x := rfl

lemma finsupp.sum_elim_inr {α β γ : Type*} [has_zero γ]
  (f : α →₀ γ) (g : β →₀ γ) (x : β) : finsupp.sum_elim f g (sum.inr x) = g x := rfl

/-- The equivalence between `(α ⊕ β) →₀ γ` and `(α →₀ γ) × (β →₀ γ)`. -/
@[simps apply symm_apply]
def finsupp.sum_arrow_equiv_prod_arrow {α β γ : Type*} [has_zero γ] :
  ((α ⊕ β) →₀ γ) ≃ (α →₀ γ) × (β →₀ γ) :=
{ to_fun := λ f,
    ⟨f.comap_domain sum.inl (sum.inl_injective.inj_on _),
     f.comap_domain sum.inr (sum.inr_injective.inj_on _)⟩,
  inv_fun := λ fg, finsupp.sum_elim fg.1 fg.2,
  left_inv := λ f, by { ext ab, cases ab with a b; simp },
  right_inv := λ fg, by { ext; simp } }

lemma finsupp.fst_sum_arrow_equiv_prod_arrow {α β γ : Type*} [has_zero γ]
  (f : (α ⊕ β) →₀ γ) (x : α) :
  (finsupp.sum_arrow_equiv_prod_arrow f).1 x = f (sum.inl x) :=
rfl

lemma finsupp.snd_sum_arrow_equiv_prod_arrow {α β γ : Type*} [has_zero γ]
  (f : (α ⊕ β) →₀ γ) (y : β) :
  (finsupp.sum_arrow_equiv_prod_arrow f).2 y = f (sum.inr y) :=
rfl

lemma finsupp.sum_arrow_equiv_prod_arrow_symm_inl {α β γ : Type*} [has_zero γ]
  (fg : (α →₀ γ) × (β →₀ γ)) (x : α) :
  (finsupp.sum_arrow_equiv_prod_arrow.symm fg) (sum.inl x) = fg.1 x :=
rfl

lemma finsupp.sum_arrow_equiv_prod_arrow_symm_inr {α β γ : Type*} [has_zero γ]
  (fg : (α →₀ γ) × (β →₀ γ)) (y : β) :
  (finsupp.sum_arrow_equiv_prod_arrow.symm fg) (sum.inr y) = fg.2 y :=
rfl
...
@[simps apply symm_apply] def finsupp.sum_arrow_lequiv_prod_arrow {α β : Type*} :
  ((α ⊕ β) →₀ M) ≃ₗ[R] (α →₀ M) × (β →₀ M) :=
{ map_add' :=
    by { intros, ext;
          simp only [equiv.to_fun_as_coe, prod.fst_add, prod.snd_add, finsupp.add_apply,
              finsupp.snd_sum_arrow_equiv_prod_arrow, finsupp.fst_sum_arrow_equiv_prod_arrow] },
  map_smul' :=
    by { intros, ext;
          simp only [equiv.to_fun_as_coe, prod.smul_fst, prod.smul_snd, finsupp.smul_apply,
              finsupp.snd_sum_arrow_equiv_prod_arrow, finsupp.fst_sum_arrow_equiv_prod_arrow] },
  .. finsupp.sum_arrow_equiv_prod_arrow }

lemma finsupp.fst_sum_arrow_lequiv_prod_arrow {α β : Type*}
  (f : (α ⊕ β) →₀ M) (x : α) :
  (finsupp.sum_arrow_lequiv_prod_arrow R f).1 x = f (sum.inl x) :=
rfl

lemma finsupp.snd_sum_arrow_lequiv_prod_arrow {α β : Type*}
  (f : (α ⊕ β) →₀ M) (y : β) :
  (finsupp.sum_arrow_lequiv_prod_arrow R f).2 y = f (sum.inr y) :=
rfl

lemma finsupp.sum_arrow_lequiv_prod_arrow_symm_inl {α β : Type*}
  (fg : (α →₀ M) × (β →₀ M)) (x : α) :
  ((finsupp.sum_arrow_lequiv_prod_arrow R).symm fg) (sum.inl x) = fg.1 x :=
rfl

lemma finsupp.sum_arrow_lequiv_prod_arrow_symm_inr {α β : Type*}
  (fg : (α →₀ M) × (β →₀ M)) (y : β) :
  ((finsupp.sum_arrow_lequiv_prod_arrow R).symm fg) (sum.inr y) = fg.2 y :=
rfl
