...
begin
  obtain ⟨⟨ι, ⟨v'⟩⟩, e : dim K V = _⟩ := cardinal.min_eq (nonempty_subtype.2 (exists_basis K V)) _,
...
calc cardinal.lift.{v v'} (dim K V) = cardinal.lift.{v v'} (cardinal.mk _) :
  congr_arg _ b.mk_eq_dim''.symm
... = cardinal.lift.{v' v} (dim K V') : (b.map f).mk_eq_dim
...
lemma exists_basis_fintype (h : dim K V < cardinal.omega) :
...
lemma exists_basis_finite (h : dim K V < cardinal.omega) :
...
def basis.of_dim_eq_zero {ι : Type*} (h : ¬ nonempty ι) (hV : dim K V = 0) :
  basis ι K V :=
begin
...
@[simp] lemma basis.of_dim_eq_zero_apply {ι : Type*} (h : ¬ nonempty ι) (hV : dim K V = 0) (i) :
  basis.of_dim_eq_zero h hV i = 0 :=
rfl

def basis.of_dim_eq_zero' (hV : dim K V = 0) :
  basis (fin 0) K V :=
basis.of_dim_eq_zero (finset.univ_eq_empty.mp rfl) hV

@[simp] lemma basis.of_dim_eq_zero'_apply (hV : dim K V = 0) (i) :
