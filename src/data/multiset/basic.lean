...
lemma mem_of_mem_nsmul {a : α} {s : multiset α} {n : ℕ} (h : a ∈ n •ℕ s) : a ∈ s :=
begin
  classical,
...
@[simp]
lemma mem_nsmul {a : α} {s : multiset α} {n : ℕ} (h0 : n ≠ 0) : a ∈ n •ℕ s ↔ a ∈ s :=
begin
...
lemma nsmul_repeat {a : α} (n m : ℕ) : n •ℕ (repeat a m) = repeat a (n * m) :=
begin
  rw eq_repeat,
  split,
